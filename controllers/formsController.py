...
from app import db
from app.models import CC, Light, Mobile, Room, Satellite, Song
...
def connectSat(request):
    ...
    s = Satellite.query.filter(
        Satellite.ip == ip).filter(
        Satellite.port == port)
...
def connectSong(request):
    ...
    song = Song.query.filter(
        Song.title == title).filter(
        Song.artist == artist).filter(
            Song.link == link)
    if not song.first():
