...
from app import db
from app.models import CC, CurrentSignals, Mobile, Room, Satellite
...
def UpdateLocationData(json):
    ...
    satellite = db.session.query(
        Satellite.roomId).filter(
        Satellite.name == satelliteName).first()
    currentSignal = db.session.query(
        CurrentSignals, Mobile.name).join(
        Mobile, Mobile.id == CurrentSignals.mobileId).filter(
            CurrentSignals.roomId == satellite.roomId).all()
...
def CreateSignal(currentSignals, satellite, deviceName, deviceRssi):
    ...
    if not any(sig for sig, name in currentSignals if (
            sig.mobileId == mobileId and sig.roomId == satellite.roomId)):
        s = CurrentSignals(
            mobileId=mobileId,
            roomId=satellite.roomId,
            rssi=deviceRssi,
...
def GetCurrentRoomIds():
    current_signals = db.session.query(
        CurrentSignals.roomId, func.max(
            CurrentSignals.rssi)).group_by(
        CurrentSignals.mobileId).all()
